...
function GradeGrid() {
  ...
  function set(student: string, subject: string, value: number) {
    const next = {
      ...grid,
      [klass]: { ...data, [student]: { ...data[student], [subject]: value } },
    };
    setGrid(next);
  }
  ...
  function exportCSV() {
    const rows = [
      ["Student", ...subjects, "Average"],
      ...Object.entries(data).map(([s, subs]) => [
        s,
        ...subjects.map((sub) => subs[sub] ?? ""),
        avg[s],
      ]),
    ];
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <PageHeader
        icon={Grid3x3}
        title="Grade Grid"
        description="Unified class grade overview — editable, failing scores highlighted in red."
        actions={
          <button
            onClick={exportCSV}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/40 text-left text-xs text-muted-foreground">
                ...
                {subjects.map((s) => (
                  <th key={s} className="px-2 text-center">
                    {s}
                  </th>
                ))}
                <th className="px-2 text-center">Avg</th>
              </tr>
            </thead>
            <tbody>
              {Object.entries(data).map(([student, subs]) => (
                <tr key={student} className="border-b border-white/20">
                    ...
                    return (
                      <td key={sub} className="px-1 text-center">
                        <input
                          ...
                          value={v}
                          onChange={(e) =>
                            set(student, sub, Math.max(0, Math.min(100, +e.target.value)))
                          }
                  ...
                  <td
                    className={`px-2 text-center font-bold ${avg[student] < 50 ? "text-destructive" : "text-mint-foreground"}`}
                  >
                    {avg[student]}
                  </td>
                </tr>
