...
function Meetings() {
  ...
  const [f, setF] = useState<Omit<Meeting, "id" | "slug">>({
    title: "",
    agenda: "",
    klass: CLASSES[0],
    subject: SUBJECTS[0],
  ...
  function link(m: Meeting) {
    ...
    const clean = /id-preview|sandbox\.lovable\.dev|--/.test(o)
      ? "https://schlaros.lovable.app"
      : o;
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <PageHeader
        ...
        description="Create classes, share links, run live sessions."
      />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Agenda / Description">
            <textarea
              ...
              placeholder="Used to generate slug URL"
            />
          </Field>
          <Field label="Meeting Mode">
            <div className="flex gap-2">
              {(["video", "audio"] as const).map((m) => (
                <button
                  ...
                  className={`flex-1 rounded-xl border px-3 py-2.5 text-sm font-semibold ${f.mode === m ? "bg-gradient-primary border-transparent text-white" : "border-white/40 bg-white/60"}`}
                >
                  {m === "video" ? (
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Audio-only recommended for slow networks.
            </p>
          </Field>
        </div>
        <div className="mt-5">
          <button
            onClick={create}
            className="bg-gradient-primary glow-primary inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-semibold text-white"
          >
            <Plus className="h-4 w-4" /> Create &amp; Start Meeting
          </button>
        </div>
      </Section>
        ...
        {meetings.length === 0 ? (
          ...
          <ul className="space-y-3">
            {meetings.map((m) => (
              <li
                key={m.id}
                className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/40 bg-white/60 p-4"
              >
                <div>
                  <div className="font-semibold">{m.title}</div>
                  <div className="text-xs text-muted-foreground">
                    {m.klass} • {m.subject} • {m.mode === "video" ? "Video" : "Audio"}
                    {m.scheduledAt && (
                      <>
                        {" "}
                ...
                <div className="flex gap-2">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="bg-gradient-primary inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-xs font-semibold text-white"
                  >
                    Start
                  </a>
                  <button
                    onClick={() => navigator.clipboard.writeText(link(m))}
                    className="inline-flex items-center gap-1 rounded-lg bg-white/70 px-3 py-1.5 text-xs font-semibold"
                  >
