...
function ParentAccess() {
  ...
  const [msgs, setMsgs] = usePersisted<Msg[]>("tg.parent-msgs", [
    {
      ...
      body: "Please share this term's calendar.",
  ...
  function bulk() {
    const fresh = studentsFor(klass).map((s) => ({
      id: crypto.randomUUID(),
      student: s,
      klass,
      code: genCode(),
  ...
  function exportCSV() {
    const rows = [
      ["Student", "Class", "Code", "Created", "Used"],
      ...codes.map((c) => [c.student, c.klass, c.code, c.createdAt, c.used ? "Yes" : "No"]),
    ];
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <PageHeader
        icon={KeyRound}
        title="Parent Access Manager"
        description="Generate access codes and manage parent messages."
        actions={
          <>
            <button
              onClick={() => navigator.clipboard.writeText(portal)}
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-white/40 bg-white/60 px-4 py-2 text-sm font-semibold"
            >
              <Download className="h-4 w-4" /> Export Codes
            </button>
          </>
        }
      />
      ...
      <Section title="Parent Portal URL">
        <div className="flex flex-wrap items-center gap-2 rounded-xl border border-white/40 bg-white/60 p-3 text-sm">
          ...
          <button
            onClick={() => navigator.clipboard.writeText(portal)}
            className="ml-auto inline-flex items-center gap-1 rounded-lg bg-gradient-primary px-3 py-1.5 text-xs font-semibold text-white"
          >
      ...
      <div className="inline-flex rounded-xl bg-white/50 p-1 text-sm">
        {[
          ...
          <button
            ...
            className={`rounded-lg px-3 py-1.5 ${tab === k ? "bg-gradient-primary text-white" : ""}`}
          >
            {l}
          </button>
        ))}
        ...
        <Section title="Generate Single Code">
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="flex items-end">
              <button
                onClick={single}
                className="bg-gradient-primary inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-white"
              >
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="flex items-end md:col-span-2">
              <button
                ...
                className="bg-gradient-primary inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-white"
              >
                <Plus className="h-4 w-4" /> Generate for {studentsFor(klass).length} Students
              </button>
            </div>
          </div>
        </Section>
                ...
                <tbody>
                  {codes.map((c) => (
                    <tr key={c.id} className="border-b border-white/20">
                      ...
                      <td>
                        {c.used ? (
                          <span className="text-mint-foreground">Used</span>
                        ) : (
                      ...
                      <td>
                        <button
                          onClick={() => setCodes(codes.filter((x) => x.id !== c.id))}
          ...
          {msgs.length === 0 ? (
            ...
            <ul className="space-y-3">
              {msgs.map((m) => (
                <li
                  key={m.id}
                  className="flex gap-3 rounded-2xl border border-white/40 bg-white/60 p-3"
                >
                  <div className="bg-gradient-primary flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-white">
