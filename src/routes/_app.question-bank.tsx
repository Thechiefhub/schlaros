...
function QuestionBank() {
  ...
  async function generate() {
    ...
    try {
      ...
      const obj = JSON.parse(res.json) as {
        questions: { q: string; options: string[]; answer: string }[];
      };
      const fresh: Q[] = obj.questions.map((q) => ({
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <PageHeader
        icon={Database}
        title="Question Bank"
        description="Reusable question database organized by subject and topic."
      />
      ...
      >
        {filtered.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No questions yet. Build your bank by generating with AI.
          </p>
        ) : (
            ...
            {filtered.map((q) => (
              <li key={q.id} className="rounded-2xl border border-white/40 bg-white/60 p-4">
                <div className="mb-2 flex flex-wrap items-center gap-2 text-xs">
                  ...
                  <button
                    onClick={() => setBank(bank.filter((x) => x.id !== q.id))}
                ...
                {q.options && (
                  <ul className="mt-2 grid gap-1.5 text-sm md:grid-cols-2">
                    {q.options.map((o, i) => (
                      <li
                        key={i}
                        className={`rounded-lg border px-3 py-1 ${o === q.answer ? "border-mint bg-mint/10 font-semibold" : "border-white/40"}`}
                      >
                        {o}
                      </li>
                    ))}
