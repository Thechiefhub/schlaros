...
function seedScores(students: string[]): Record<string, number> {
  return Object.fromEntries(
    students.map((s, i) => [s, Math.max(20, Math.min(99, 50 + ((i * 17) % 50)))]),
  );
}
...
function SeatingChart() {
  ...
  const chart: Chart = store[klass] ?? {
    rows: 5,
    cols: 6,
    seats: Array(30).fill(null),
    scores: seedScores(students),
  };

  function update(next: Chart) {
    setStore({ ...store, [klass]: next });
  }
  function applyGrid(rows: number, cols: number) {
    update({ ...chart, rows, cols, seats: Array(rows * cols).fill(null) });
  }
  function autoAssign() {
    const seats = [...chart.seats];
    let i = 0;
    students.forEach((s) => {
      while (i < seats.length && seats[i] !== null) i++;
      if (i < seats.length) seats[i++] = s;
    });
    update({ ...chart, seats });
  }
  function shuffle() {
  ...
  function clear() {
    update({ ...chart, seats: Array(chart.rows * chart.cols).fill(null) });
  }
  ...
  const unseated = useMemo(
    () => students.filter((s) => !chart.seats.includes(s)),
    [students, chart.seats],
  );
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <PageHeader
        icon={Armchair}
        title="Seating Chart"
        description="Color-coded by latest assessment average. Assign seats manually or auto."
      />
      ...
      <Section title="Chart Settings">
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Field label="Rows">
            <input
              ...
              className="input"
              value={chart.rows}
              onChange={(e) => applyGrid(+e.target.value, chart.cols)}
            />
          </Field>
          <Field label="Cols">
            <input
              ...
              className="input"
              value={chart.cols}
              onChange={(e) => applyGrid(chart.rows, +e.target.value)}
            />
          </Field>
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
          <button
            onClick={autoAssign}
            className="bg-gradient-primary inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold text-white"
          >
      ...
      <Section title={`Classroom — ${name}`}>
        <div className="overflow-x-auto">
          <div
            ...
            style={{ gridTemplateColumns: `repeat(${chart.cols}, minmax(120px, 1fr))` }}
          >
            {chart.seats.map((s, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`flex h-24 flex-col items-center justify-center rounded-2xl border-2 p-2 text-center text-xs transition ${toneFor(score)}`}
                >
                  <select
                  ...
                  {score !== undefined && (
                    <span className="mt-1 text-[10px] opacity-80">{score}%</span>
                  )}
                </div>
              );
            })}
...
function Legend({ tone, label }: { tone: "mint" | "amber" | "destructive"; label: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-2">
      <span className={`h-3 w-3 rounded-full ${map[tone]}`} /> {label}
    </span>
  );
}
